...
<main class="h-full w[50%] bg-gray-50 flex flex-wrap items-center justify-center overflow-x-hidden transition-transform duration-300 ease-in-out">
    ...
    @foreach($saved as $index => $save)
    <div class="border max-w-screen-md w-full bg-white mt-6 rounded-2xl p-4 mb-5">
      <div class="flex items-center justify-between relative">
          <div class="gap-3.5	flex items-center">
              <img src="{{asset('' . $save->posts->users->avatar)}}" class="object-cover bg-yellow-500 rounded-full w-10 h-10" />
              <div class="flex flex-col">
                  <b class="mb-2 capitalize">{{$save->posts->users->user_name}}</b>
                  @if(isset($follow->id))
                    @foreach ($followings as $following)
                      @if ($save->posts->user_id != Auth::user()->id && $save->posts->user_id != $following->friend_id)
                          <form action="{{route('follow', $save->posts->user_id)}}" method="GET">
                            @csrf
          ...
          {{-- <div x-data="{ openSettings{{$index}}: false }">
              ...
              <div x-show="openSettings{{$index}}" @click.away="openSettings{{$index}} = false" class="bg-white absolute right-0 top-10 w-40 py-2 mt-1 border border-gray-200 shadow-2xl">
                  ...
                  <div class="py-2">
                      <form action="{{route('deletePost', $save->posts->id)}}">
                        @csrf
                      <button type="submit" class="w-full flex items-center py-1.5 px-6 space-x-2 hover:bg-gray-200">
                        <span class="material-symbols-outlined  h-3 w-3 text-gray-400 flex justify-center items-center">
                          delete
      ...
      <button onclick="toggleDescription({{ $save->posts->id }})">Show More</button>
        <div class="mt-5 flex gap-2	 justify-center border-b pb-4 flex-wrap	">
          @foreach($save->posts->photos as $photo)
          ...
          <div class=" h-16 border-b  flex items-center justify-around">
              ...
              <div class="flex items-center	gap-3">
                  <form action="{{route('postVote', $save->posts->id)}}" method="GET">
                      @csrf
              ...
              <div class="flex items-center	gap-3">
                  <button id="postId_button"  data-post-id="{{$save->posts->id}}">
  ...
  $(document).ready(function(){
    $(document).on('click', '#postId_button', function(){
      ...
      var saved = document.getElementById('savedButton')
      console.log(postId);
      $.ajax({
          ...
          success: function(data) {
            if(data.success == true){
              console.log(data);
              saved.style.color = '#FF0000';
            }
              alert('added to favorities list successfully');
          }
      });
    });
  });
